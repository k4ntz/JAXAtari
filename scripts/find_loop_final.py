...
def find_loop():
    ...
    for length in range(10, 2000):
        # We want a loop that repeats perfectly for a long time
        if line_strs[-length:] == line_strs[-2*length:-length] == line_strs[-3*length:-2*length]:
            print(f"Candidate loop of length {length}")
            # Find when it starts
...
if __name__ == "__main__":
    ...
    if init is not None:
        ...
        np.save("beamrider_init_final.npy", np.array([pad(l) for l in init]))
        np.save("beamrider_loop_final.npy", np.array([pad(l) for l in loop]))
