...
from jaxatari.wrappers import AtariWrapper, PixelObsWrapper, NormalizeObservationWrapper, FlattenObservationWrapper
...
def run_jax_benchmark(
    ...
    def run_one_step_render(carry, _):
        ...
        check_sum = jnp.sum(rendered_images)

        return (final_next_states, next_rng_key), (rewards, terms, check_sum)
    ...
    if use_render:
        ...
        # Warmup phase
        print(f"JAX: Warming up with {warmup_steps} steps...")
        (w_states, w_key), _ = jax.lax.scan(run_one_step_render, (c_states, run_key), None, length=warmup_steps)
        jax.block_until_ready((w_states, w_key))
        print(f"JAX: Warmup completed.")
        time.sleep(0.5)  # Brief pause after warmup

        # Run benchmark
        monitor.start()
        execution_start_time = time.time()
        (f_states, _), (_, _, f_images) = jax.lax.scan(run_one_step_render, (w_states, w_key), None, length=num_steps_per_env)
        ...
        # Warmup phase
        print(f"JAX: Warming up with {warmup_steps} steps...")
        (w_states, w_key), _ = jax.lax.scan(run_one_step_no_render, (c_states, run_key), None, length=warmup_steps)
        jax.block_until_ready(w_states)
        print(f"JAX: Warmup completed.")
        time.sleep(0.5)  # Brief pause after warmup
        ...
        monitor.start()
        execution_start_time = time.time()
        (f_states, _), _ = jax.lax.scan(run_one_step_no_render, (w_states, w_key), None, length=num_steps_per_env)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--steps-per-env", type=int, default=25_000,
    ...
    parser.add_argument(
        "--env-counts", type=int, nargs='+', default=[64, 128],
    ...
    parser.add_argument(
        ...
        help="Number of warmup steps to run after compilation and before timing (default: 1000)."
    )
