...
def find_loop_robust():
    ...
    for length in range(10, 1000):
        # Check last 4 repetitions
        if (line_strs[-length:] == line_strs[-2*length:-length] == 
            line_strs[-3*length:-2*length] == line_strs[-4*length:-3*length]):
            print(f"Found robust loop of length {length}")
            # Find the start of this repeating pattern
            # Starting from frame 128 (approx game start)
            for start in range(128, len(all_lines) - 2*length):
                if line_strs[start:start+length] == line_strs[start+length:start+2*length]:
                    print(f"Loop starts at {start}")
                    return all_lines[:start], all_lines[start:start+length]
    return None, None

if __name__ == "__main__":
    init, loop = find_loop_robust()
    if init:
        print(f"Init: {len(init)}, Loop: {len(loop)}")
        def pad(l): return l + [-1] * (7 - len(l))
        np.save("beamrider_init_final_v2.npy", np.array([pad(l) for l in init]))
        np.save("beamrider_loop_final_v2.npy", np.array([pad(l) for l in loop]))
