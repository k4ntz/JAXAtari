...
def find_loop_from_start():
    ...
    # We know the game "starts" around 128. 
    # Let's see if the lines loop earlier.
    # Try different loop lengths
    for length in range(10, 1000):
        # Look at the last 3 loops
        if line_strs[-length:] == line_strs[-2*length:-length] == line_strs[-3*length:-2*length]:
            print(f"Loop of length {length} confirmed.")
...
if __name__ == "__main__":
    init, loop = find_loop_from_start()
    if init:
        print(f"Init frames: {len(init)}, Loop frames: {len(loop)}")
        def pad(l): return l + [-1] * (7 - len(l))
        np.save("beamrider_init_fixed_v3.npy", np.array([pad(l) for l in init]))
        np.save("beamrider_loop_fixed_v3.npy", np.array([pad(l) for l in loop]))
