...
class JaxBreakout(
    JaxEnvironment[BreakoutState, BreakoutObservation, BreakoutInfo, BreakoutConstants]
):
    def __init__(
        self, consts: BreakoutConstants = None, reward_funcs: list[callable] = None
    ):
    ...
    def _player_step(
        ...
        touches_wall = jnp.logical_or(
            state_player_x <= self.consts.PLAYER_X_MIN,
            state_player_x >= self.consts.PLAYER_X_MAX,
        )
    ...
    def _detect_paddle_hit(self, ball_x, ball_y, player_x, small_paddle):
        ...
        paddle_width = jnp.where(
            small_paddle, self.consts.PLAYER_SIZE_SMALL[0], self.consts.PLAYER_SIZE[0]
        )
        ...
        paddle_hit_y_from_above = jnp.logical_and(
            ...
            ball_y + self.consts.BALL_SIZE[1]
            <= self.consts.PLAYER_START_Y + self.consts.PLAYER_SIZE[1],
        )
        ...
        paddle_hit_x_from_left = jnp.logical_and(
            ...
            ball_x + self.consts.BALL_SIZE[0]
            <= player_x + 2,  # Small tolerance for side hit
        )
        ...
        return jnp.logical_or(
            jnp.logical_or(hit_from_above, hit_from_left), hit_from_right
        )
    ...
    def _ball_step(self, state, game_started, player_x):
        ...
        def not_started_fn(_):
            ...
            new_speed_idx = 0
            vel_x, vel_y = self._get_ball_velocity(
                new_speed_idx, new_direction_idx, state.step_counter
            )
            return (
                ball_start_x,
                ball_start_y,
        ...
        def started_fn(_):
            ...
            right_collision = (
                ball_x >= 160 - self.consts.WALL_SIDE_WIDTH - self.consts.BALL_SIZE[0]
            )
            ball_x = jnp.where(left_collision, self.consts.WALL_SIDE_WIDTH, ball_x)
            ball_x = jnp.where(
                right_collision,
                160 - self.consts.WALL_SIDE_WIDTH - self.consts.BALL_SIZE[0],
                ball_x,
            )
            ...
            ball_in_block_area = jnp.logical_and(
                ...
                ball_y
                <= self.consts.BLOCK_START_Y
                + self.consts.NUM_ROWS * self.consts.BLOCK_SIZE[1],
            )
            hit_wall_or_paddle = jnp.logical_or(
                hit_wall_or_paddle,
                jnp.logical_and(hit_side_wall, jnp.logical_not(ball_in_block_area)),
            )

            # Collision handling with top wall
            top_collision = (
                ball_y <= self.consts.WALL_TOP_Y + self.consts.WALL_TOP_HEIGHT
            )
            ball_y = jnp.where(
                top_collision,
            ...
            section_width = (
                self.consts.PLAYER_SIZE[0] / 5
            )  # Divide section into 5 equal parts
            ...
            ball_speed_idx = jax.lax.select(
                jnp.logical_and(
                    paddle_hit, jnp.logical_and(hit_section == 0, ball_speed_idx != 4)
                ),
                1,
                ball_speed_idx,
            )
            ...
            ball_speed_idx = jax.lax.select(
                jnp.logical_and(
                    paddle_hit, jnp.logical_and(hit_section == 1, ball_speed_idx != 4)
                ),
                2,
                ball_speed_idx,
            )
            ...
            ball_speed_idx = jax.lax.select(
                jnp.logical_and(
                    paddle_hit, jnp.logical_and(hit_section == 2, ball_speed_idx != 4)
                ),
                2,
                ball_speed_idx,
            )
            ...
            ball_speed_idx = jax.lax.select(
                jnp.logical_and(
                    paddle_hit, jnp.logical_and(hit_section == 3, ball_speed_idx != 4)
                ),
                2,
                ball_speed_idx,
            )
            ...
            ball_speed_idx = jax.lax.select(
                jnp.logical_and(
                    paddle_hit, jnp.logical_and(hit_section == 4, ball_speed_idx != 4)
                ),
                1,
                ball_speed_idx,
            )
            ...
            new_vel_x, new_vel_y = self._get_ball_velocity(
                ball_speed_idx, ball_direction_idx, state.step_counter
            )

            paddle_hit_for_hit_counter = jnp.logical_and(
                paddle_hit, jnp.logical_and(state.ball_vel_y > 0, new_vel_y < 0)
            )
        ...
        (
            ...
            blocks_hittable,
            small_paddle,
        ) = jax.lax.cond(game_started, started_fn, not_started_fn, operand=None)
    ...
    def _check_block_collision(
        ...
        def hit_path(_):
            ...
            row_indices, col_indices = jnp.mgrid[
                : self.consts.NUM_ROWS, : self.consts.BLOCKS_PER_ROW
            ]

            # Calculate positions for all blocks at once
            block_xs = (
                self.consts.BLOCK_START_X + col_indices * self.consts.BLOCK_SIZE[0]
            )
            block_ys = (
                self.consts.BLOCK_START_Y + row_indices * self.consts.BLOCK_SIZE[1]
            )
            ...
            collision_mask = jnp.logical_and(
                active_blocks, jnp.logical_and(x_collision, y_collision)
            )

            # Check for bottom edge collision (for bounce)
            bounce_condition = (
                jnp.abs((block_ys + self.consts.BLOCK_SIZE[1]) - ball_y) <= 4
            )
            ...
            masked_priorities = jnp.where(
                ...
                priority_grid,
                jnp.full_like(
                    priority_grid, self.consts.NUM_ROWS * self.consts.BLOCKS_PER_ROW + 1
                ),  # High value for non-hits
            )
            ...
            points = jnp.where(
                hit_row >= 4,
                1,  # Bottom rows
                jnp.where(hit_row >= 2, 4, 7),  # Middle rows  # Top rows
            )
            ...
            updated_score = jnp.where(
                jnp.logical_and(any_hit, valid_hit), score + points, score
            )
            ...
            accelerate = jnp.logical_and(
                jnp.logical_and(any_hit, valid_hit), hit_row < 3
            )
            ...
            new_vel_x, new_vel_y = self._get_ball_velocity(
                updated_speed_idx, updated_direction_idx, state.step_counter
            )

            # Update blocks_hittable
            updated_blocks_hittable = jnp.where(
                jnp.logical_and(any_hit, valid_hit), False, blocks_hittable
            )
            ...
            return (
                ...
                new_vel_y,
                updated_speed_idx,
                updated_direction_idx,
                consecutive_hits,
                updated_blocks_hittable,
                all_blocks_cleared,
            )

        # Execute the appropriate path based on blocks_hittable
        return jax.lax.cond(blocks_hittable, hit_path, no_hit_path, operand=None)
    ...
    def reset(self, key: chex.PRNGKey = None) -> tuple[BreakoutObservation, BreakoutState]:
        ...
        init_vel_x, init_vel_y = self._get_ball_velocity(
            init_speed_idx, init_direction_idx, 0
        )
        ...
        state = BreakoutState(
            ...
            blocks=jnp.ones(
                (self.consts.NUM_ROWS, self.consts.BLOCKS_PER_ROW), dtype=jnp.int32
            ),
            score=jnp.array(0),
    ...
    @partial(jax.jit, static_argnums=(0,))
    def step(
        self, state: BreakoutState, action: chex.Array
    ) -> Tuple[
    ...
    def _step(self, state: BreakoutState, action: chex.Array) -> BreakoutState:
        ...
        (
            ...
            blocks_hittable,
            small_paddle,
        ) = self._ball_step(state, game_started, new_player_x)
        ...
        (
            ...
            state._replace(blocks_hittable=blocks_hittable),
            ball_x,
            ball_y,
            ball_speed_idx,
            ball_direction_idx,
        ...
        should_reset_wall = jnp.logical_and(
            jnp.logical_and(state.all_blocks_cleared, paddle_hit), state.wall_resets < 1
        )
        new_blocks = jnp.where(should_reset_wall, jnp.ones_like(new_blocks), new_blocks)
        new_wall_resets = jnp.where(
            should_reset_wall, state.wall_resets + 1, state.wall_resets
        )
        ...
        new_all_blocks_cleared = jnp.logical_and(
            ...
            jnp.logical_not(
                should_reset_wall
            ),  # Reset to False if we just reset the wall
        )
        ...
        ball_vel_x, ball_vel_y = self._get_ball_velocity(
            ball_speed_idx, ball_direction_idx, state.step_counter
        )
    ...
    def _get_observation(self, state: BreakoutState) -> BreakoutObservation:
        paddle_width = jnp.where(
            state.small_paddle,
            self.consts.PLAYER_SIZE_SMALL[0],
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _get_info(
        self, state: BreakoutState, all_rewards: chex.Array = None
    ) -> BreakoutInfo:
        return BreakoutInfo(
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _get_reward(
        self, previous_state: BreakoutState, current_state: BreakoutState
    ) -> chex.Array:
    ...
    def _get_done(self, state: BreakoutState) -> chex.Array:
        return jnp.logical_or(
            state.lives <= 0,
            jnp.logical_or(state.all_blocks_cleared, state.step_counter >= 5000),
        )
        ...
        return spaces.Dict(
            {
                ...
                "blocks": spaces.Box(
                    low=0,
                    high=1,
                    shape=(self.consts.NUM_ROWS, self.consts.BLOCKS_PER_ROW),
                    dtype=jnp.int32,
                ),
...
class BreakoutRenderer(JAXGameRenderer):
    ...
    def render(self, state):
        ...
        raster = jr.render_at(
            raster, state.player_x, self.consts.PLAYER_START_Y, frame_player
        )
        ...
        raster = jr.render_label_selective(
            raster, 36, 5, player_score_digits, self.DIGIT_SPRITES, 0, 3, spacing=16
        )

        # 2. Render number of lives
        raster = jr.render_label_selective(
            raster, 100, 5, player_lifes_digit, self.DIGIT_SPRITES, 0, 1, spacing=16
        )

        # 3. Render number of players
        raster = jr.render_label_selective(
            raster, 132, 5, number_players_digit, self.DIGIT_SPRITES, 0, 1, spacing=16
        )
