...
DEFAULT_DICE = 2   # Normally 2 dice are rolled, but 4 on doubles
# Player constants
...
class BackgammonState(NamedTuple):
    ...
    Attributes:
        ...
        dice: A (4,) array of dice values.
              - Normally: [d1, d2, 0, 0]
              - On doubles: [d, d, d, d]
              - Used dice are marked as 0 after being consumed
        current_player: WHITE (1) or BLACK (-1)
        is_game_over: True if one player has borne off all 15 checkers
    """
    board: jnp.ndarray  # Shape (2, 26): checkers per point/bar/home
    dice: jnp.ndarray   # Shape (4,): dice values (0s mean already used)
    current_player: int
    is_game_over: bool
...
def reset(key: jax.Array, starting_player: int = WHITE) ->  Tuple[BackgammonState, KeyArray]:
    """Reset the game to the initial state and roll initial dice."""
    state = init_state()
    dice, key = roll_dice(key)
    state = state._replace(dice=dice, current_player=starting_player)
    return state, key
# NOTE: In JAX, random number generation is purely functional.
# You must pass in a PRNG key and split it every time you use randomness
# (e.g., to roll dice). After splitting, you MUST return the new key.
# This ensures future random calls are safe, reproducible, and follow JAX's
# rule of no hidden state or side effects.


def roll_dice(key: jax.Array) -> Tuple[jnp.ndarray, jax.Array]:
    key, subkey = jax.random.split(key)
    dice = jax.random.randint(subkey, (DEFAULT_DICE,), 1, 7)

    is_double = dice[0] == dice[1]
    extended_dice = jax.lax.cond(
        is_double,
        lambda d: jnp.array([d[0], d[0], d[0], d[0]]),  # 4 moves
        lambda d: jnp.array([d[0], d[1], 0, 0]),        # 2 moves, pad with zeros
        operand=dice
    )
    return extended_dice, key
...
def is_valid_move(state: BackgammonState, from_point: int, to_point: int) -> bool:
    ...
    # fourth condition: check if the move is valid according to the dice; returns True if any die equals the distance
    ...
    has_bar_pieces = board[player_idx, BAR_INDEX] > 0

    #If player has bar pieces, only allow moves from the bar
    must_move_from_bar = (~has_bar_pieces) | (from_point == BAR_INDEX)

    # Sixth condition: for bar moves, only allow if dice matches expected entry point AND that point is not blocked
    def is_valid_bar_entry(dice_val: int) -> bool:
        expected_point = jax.lax.cond(
            player == WHITE,
            lambda _: 24 - dice_val,
            lambda _: dice_val - 1,
            operand=None
        )
        # FIRST: check if this die allows entry to this to_point
        matches_point = to_point == expected_point
        # THEN: check if that point is not blocked by opponent
        point_is_open = board[opponent_idx, expected_point] <= 1
        return matches_point & point_is_open

    bar_entry_valid = jax.lax.cond(
        from_point == BAR_INDEX,
        lambda _: jnp.any(jax.vmap(is_valid_bar_entry)(state.dice)),
        lambda _: True,
        operand=None
    )
