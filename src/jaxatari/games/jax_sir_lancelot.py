...
class SirLancelotConstants(NamedTuple):
    ...
    MAX_VEL_Y_UP: float = -2.4          # Much higher ceiling for momentum stacking
    MAX_VEL_Y_DOWN: float = 1.2         # Much faster falling speed
    ...
    MAX_VEL_X: float = 5.0                 # Even higher horizontal speed limit
    ...
    DRAGON_SPEED_LEVEL_4: float = 0.8  # 2x speed
    DRAGON_SPEED_LEVEL_6: float = 1.2  # 3x speed
    DRAGON_SPEED_LEVEL_8: float = 1.6  # 4x speed (Hanek the Horrible)
    ...
    # Starting level
    START_LEVEL: int = 1 # Start at Level 8 (final castle level)
...
class JaxSirLancelot(JaxEnvironment[SirLancelotState, SirLancelotObservation, SirLancelotInfo, SirLancelotConstants]):
    ...
    def _apply_physics(self, player: PlayerState, action: int) -> PlayerState:
        ...
        # Collision detection (before clamping position)
        hitting_ceiling = new_y <= self.consts.PLAY_AREA_TOP
        hitting_ground = new_y >= (self.consts.PLAY_AREA_BOTTOM - self.consts.PLAYER_HEIGHT)
        
        # Ceiling bounce - reverse FULL vertical velocity (before any clamping)
        new_vel_y = jnp.where(hitting_ceiling, jnp.abs(new_vel_y), new_vel_y)
        
        # Ground collision
        new_vel_y = jnp.where(hitting_ground, 0.0, new_vel_y)
        
        # Apply ground friction when on ground
        new_vel_x = jnp.where(
            hitting_ground,
            new_vel_x * 0.7,  # Strong friction when on ground
            new_vel_x
        )
        
        # Apply velocity limits ONLY if not bouncing from ceiling
        # This preserves the full bounce force
        new_vel_y = jnp.where(
            hitting_ceiling,
            new_vel_y,  # Keep full bounce velocity
            jnp.clip(new_vel_y, self.consts.MAX_VEL_Y_UP, self.consts.MAX_VEL_Y_DOWN)
        )
        
        # Clamp position
        new_y = jnp.clip(new_y, self.consts.PLAY_AREA_TOP, self.consts.PLAY_AREA_BOTTOM - self.consts.PLAYER_HEIGHT)
    ...
    def _update_beasts(self, enemies: EnemyState, time: chex.Array, level: chex.Array, quest: chex.Array) -> EnemyState:
        ...
        def update_level_1():
            ...
            new_vel_y = jnp.zeros_like(enemies.vel_y)
        ...
        def update_level_3():
            ...
            # No vertical movement for level 3
            new_vel_y = jnp.zeros_like(enemies.vel_y)
            return new_vel_x, new_vel_y
        ...
        new_y = enemies.positions[:, 1] + new_vel_y
    ...
    def _resolve_combat(self, player: PlayerState, enemies: EnemyState, 
        ...
        e_left = enemies.positions[:, 0]
        e_right = enemies.positions[:, 0] + self.consts.ENEMY_WIDTH
        e_top = enemies.positions[:, 1]
        e_bottom = enemies.positions[:, 1] + self.consts.ENEMY_HEIGHT
        ...
        # Enemy always faces left in Level 1
        enemy_facing_player = player.x < enemies.positions[:, 0]
        
        # Combat rules
        both_facing = jnp.logical_and(player_facing_enemy, enemy_facing_player)
        player_higher = player.y < enemies.positions[:, 1]
        
        # Player wins if: facing enemy and enemy not facing, or both facing and higher
        player_wins = jnp.logical_and(
            collision_mask,
            jnp.logical_or(
                jnp.logical_and(player_facing_enemy, jnp.logical_not(enemy_facing_player)),
                jnp.logical_and(both_facing, player_higher)
            )
        )
        
        # Enemy wins if: enemy facing and player not, or both facing and enemy higher
        enemy_wins = jnp.logical_and(
            collision_mask,
            jnp.logical_or(
                jnp.logical_and(enemy_facing_player, jnp.logical_not(player_facing_enemy)),
                jnp.logical_and(both_facing, jnp.logical_not(player_higher))
            )
        )
        ...
        new_enemies = enemies._replace(active=new_enemy_active)
        ...
        base_score = num_kills * points_per_kill
    ...
    def _get_reward(self, state: SirLancelotState) -> float:
        return state.score.astype(jnp.float32)
