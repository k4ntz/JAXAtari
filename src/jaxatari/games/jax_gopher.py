...
def init_y_position_to_garden_tile_y_map():
    y_map = [3]
    for y in range(1, 7):
        y_map.append(2)
    for y in range(7, 14):
        y_map.append(1)
    for y in range(14, GOPHER_Y_ABOVE_GROUND):
        y_map.append(0)
    return y_map


Y_POSITION_TO_GARDEN_TILE_Y_MAP = byte_arr(init_y_position_to_garden_tile_y_map())


def init_garden_dirt_flags():
...
INIT_GARDEN_DIRT_FLAGS = bool_arr(init_garden_dirt_flags())


def init_dug_dirt_display_mask():
    # dirt bit at 0,0 never gets dug, so it's safe to use
    display_mask = np.zeros([WINDOW_HEIGHT, WINDOW_WIDTH])
...
DUG_DIRT_DISPLAY_MASK = int_arr(init_dug_dirt_display_mask())
...
@jit
def reset_to_main_loop(s: GopherState, duck_enabled: BoolValue, expert_difficulty: BoolValue) -> GopherState:
    s = s._replace(
        expert_difficulty=expert_difficulty,
        game_state=byte_val(GS_MAIN_GAME_LOOP),
...
@jit
def init_game_round_data(s: GopherState) -> GopherState:
    return init_garden_dirt_values(
        s._replace(
            ...
            farmer_x=byte_val(FARMER_INIT_X),
            gopher_x=byte_val(GOPHER_INIT_X),
            duck_x=byte_val(GOPHER_INIT_X),
...
@jit
def init_garden_dirt_values(s: GopherState):
    return s._replace(
        garden_dirt_flags=INIT_GARDEN_DIRT_FLAGS,
        gopher_taunt_timer=byte_val(0),
        gopher_y_target_id=jnp.bitwise_and(s.random_0, GOPHER_Y_TARGET_MASK),
        gopher_y_locked=bool_val(False),
        gopher_y_target_extra_flag=jnp.bitwise_and(s.random_0,
                                                   (~GOPHER_Y_TARGET_MASK & 0xFF & ~GOPHER_Y_LOCKED_BIT)) != 0,
        gopher_target_move_left=jnp.bitwise_and(s.random_1, GOPHER_X_DIR_MASK) != 0,
        gopher_tunnel_target_id=jnp.bitwise_and(s.random_1, GOPHER_TUNNEL_TARGET_MASK),
...
@jit
def get_garden_flag_x_index(x_pos: IntValue) -> IntValue:
    return x_pos // 4


@jit
def get_garden_flag_y_index(y_pos: IntValue) -> IntValue:
    return Y_POSITION_TO_GARDEN_TILE_Y_MAP[y_pos]


@jit
def get_garden_flag_index_from_pos(x_pos: IntValue, y_pos: IntValue) -> IntValue:
    x_idx = get_garden_flag_x_index(x_pos)
    y_idx = get_garden_flag_y_index(y_pos)

    return get_garden_flag_index(x_idx, y_idx)


@jit
def get_garden_flag_index(x_idx: IntValue, y_idx: IntValue) -> IntValue:
    return y_idx * 40 + x_idx


@jit
def update_game(s: GopherState) -> GopherState:
    # Increment frameCount
    s = s._replace(frame_count=s.frame_count + 1)

    s, carry = play_game_audio(s, s.carry_state)
    s = next_random(s, carry)
    ...
    return jif_true(
        s.game_state == GS_MAIN_GAME_LOOP,
        lambda s: update_farmer(update_gopher_digging(s)),
        operand=s
    )


@jit
def update_duck(s: GopherState) -> GopherState:
    def update_animation(s: GopherState) -> GopherState:
        ...
        s = jif_true(
            new_timer == DUCK_ANIMATION_DOWN_WING,
            lambda s: s._replace(
        ...
        s = jif_true(
            new_timer == DUCK_ANIMATION_STATIONARY_WING,
            lambda s: s._replace(
        ...
        s = jif_true(
            new_timer == DUCK_ANIMATION_UP_WING,
            lambda s: s._replace(
    ...
    def update(s: GopherState) -> GopherState:
        ...
        s = jif(
            ...
            lambda s: s._replace(
                duck_animation_timer=byte_val(DUCK_INIT_ANIMATION_TIMER),
                duck_render_state=byte_val(DuckRenderState.STATIONARY.value)
            ),
            update_animation,
            operand=s
        ...
        s = jif_true(
            ...
            lambda s: set_game_audio_values(s, MEM_AUDIO_DUCK_QUACKING - MEM_AUDIO_VALUES),
            operand=s
        )
        ...
        return jif_true(
            jor(s.duck_x < DUCK_MIN_X, s.duck_x >= DUCK_MAX_X),
            disable_duck,
            operand=s
        )
...
@jit
def disable_duck(s: GopherState) -> GopherState:
    return s._replace(
        duck_render_state=byte_val(DuckRenderState.DISABLED.value),
        duck_animation_timer=byte_val(0)
    )
...
def update_seed(s: GopherState) -> GopherState:
    ...
    return jif_true(
        ...
        lambda s: jif(
            ...
            update_seed_held_by_farmer,  # Seed is held by farmer
            lambda s: jif(
                s.duck_seed_drop_target == s.seed_x,
                update_falling_seed,  # Seed reached target position
...
@jit
def update_falling_seed(s: GopherState) -> GopherState:
    # Seed falls down
    ...
    return jif_true(
        jand(jand(
            new_seed_y >= SEED_MIN_CATCHING_Y,
            new_seed_y < SEED_MAX_CATCHING_Y
        ), jnp.abs(s.seed_x.astype(jnp.int16) - s.farmer_x) < 5),
        lambda s: s._replace(
            held_seed_decaying_timer=byte_val(SEED_INIT_DECAYING_TIMER)
        ),
        operand=s
    )
...
@jit
def update_seed_held_by_farmer(s: GopherState) -> GopherState:
...
@jit
def update_gopher_digging(s: GopherState) -> GopherState:
    s = jif_true(
        jand(
            s.score_value >= 10_000,
            jor(s.gopher_y_target_id != GOPHER_Y_TARGET_UNDER_GROUND, s.gopher_y_target_extra_flag)
        ),
        lambda s: s._replace(
            gopher_y_locked=True,
            gopher_y_target_id=jnp.bitwise_or(s.gopher_y_target_id, GOPHER_Y_DIFFICULT_TARGET_MASK)
        ),
        operand=s
    )

    return jif(
        s.gopher_y == GOPHER_Y_UNDERGROUND,
        lambda s: gopher_digging(s, s.gopher_x + jselect(s.gopher_facing_left, byte_val(0), byte_val(8))),
        lambda s: jif_true(
            s.gopher_y != GOPHER_Y_ABOVE_GROUND,
            lambda s: gopher_digging(s, TUNNEL_X_POSITIONS[s.gopher_tunnel_target_id]),
            operand=s
        ),
        operand=s
    )
...
@jit
def gopher_digging(s: GopherState, x_pos: ByteValue) -> GopherState:
    def dirt_not_removed(s: GopherState, x_pos: ByteValue) -> GopherState:
        dirt_idx = get_garden_flag_index_from_pos(x_pos, s.gopher_y)

        s = set_game_audio_values(s._replace(
            garden_dirt_flags=s.garden_dirt_flags.at[dirt_idx].set(True)
        ), MEM_AUDIO_DIG_TUNNEL - MEM_AUDIO_VALUES)

        s = jif_p2_true(
            s.gopher_y != GOPHER_Y_UNDERGROUND,
            lambda s, x_pos: s._replace(
                # Up or down movement, removing adjacent dirt as well
                garden_dirt_flags=s.garden_dirt_flags.at[get_garden_flag_index_from_pos(x_pos + 1, s.gopher_y)].set(
                    True)
            ),
            operand=(s, x_pos)
        )

        return check_to_change_gopher_horizontal_direction(s)
...
@jit
def check_to_change_gopher_horizontal_direction(s: GopherState) -> GopherState:
        ...
        return jif(
            ...
            lambda s: jif(
                ...
                lambda s: s._replace(
                    ...
                    gopher_y_locked=bool_val(True),
                    gopher_y_target_id=byte_val(GOPHER_Y_TARGET_UNDER_GROUND),
                ),
                operand=s
            ),
            operand=s
...
@jit
def update_farmer(s: GopherState) -> GopherState:
    def check_for_action(s: GopherState) -> GopherState:
        def check_already_pressed(s: GopherState):
            return jif_true(
                jnot(s.action_button_prev_pressed),
                lambda s: increment_farmer_animation_index(s._replace(action_button_prev_pressed=bool_val(True))),
                operand=s
            )
...
@jit
def farmer_action(s: GopherState) -> GopherState:
    carrot_id = get_first_index(jnp.abs((s.farmer_x.astype(jnp.int16) - 4) - CARROT_X_POSITIONS) < 6)

    def potentially_fill_hole(s: GopherState):
        tunnel_id = get_first_index(jnp.abs((s.farmer_x.astype(jnp.int16) - 4) - TUNNEL_X_POSITIONS) < 6)
        return jif(
            tunnel_id == -1,
            lambda params: params[0],
            lambda params: fill_tunnel(params[0], params[1]),
            operand=(s, tunnel_id)
        )
...
@jit
def fill_tunnel(s: GopherState, hole_id: IntValue):
    gopher_target_x = jif(
        s.gopher_targeting_carrot,
        lambda s: CARROT_X_POSITIONS[s.gopher_carrot_target_id],
        lambda s: TUNNEL_X_POSITIONS[s.gopher_tunnel_target_id],
        operand=s
    )
    filling_possible = jnot(jand(gopher_target_x == TUNNEL_X_POSITIONS[hole_id],
                                 s.gopher_y != GOPHER_Y_UNDERGROUND))
    ...
    def fill_hole(params: tuple[GopherState, IntValue, IntValue]):
        ...
        return increment_score(
            set_game_audio_values(
                s._replace(
                    garden_dirt_flags=new_garden_dirt_flags
                ),
                MEM_AUDIO_FILL_TUNNEL - MEM_AUDIO_VALUES
            ),
            POINTS_FILL_TUNNEL
        )

    def potentially_fill_hole(s: GopherState, hole_id: IntValue):
        hole_x_position = TUNNEL_X_POSITIONS[hole_id]
        hole_x_idx = get_garden_flag_x_index(hole_x_position)

        first_dirt_y_pos = get_first_dirt_index_for_hole(s, hole_x_position)
...
@partial(jit, static_argnums=(1,))
def increment_score(s: GopherState, amount: int) -> GopherState:
    ...
    s = jif_true(
        ...
        lambda s: check_to_spawn_duck(check_to_decrement_gopher_direction_timer(s)),
        operand=s
    )
...
@jit
def check_to_decrement_gopher_direction_timer(s: GopherState) -> GopherState:
...
@jit
def check_to_spawn_duck(s: GopherState) -> GopherState:
    def spawn_duck(s: GopherState) -> GopherState:
        new_attributes_byte = s.random_0
        new_moving_left = is_msb_set(new_attributes_byte)
        new_seed_target_pos = jnp.bitwise_and(new_attributes_byte, SEED_TARGET_X_MASK)

        # Spawn at left or right edge of screen
        duck_spawn_x = jselect(new_moving_left, byte_val(DUCK_MAX_X), byte_val(DUCK_MIN_X))
        # Spawn seed with specific offset to duck
        seed_spawn_x = jselect(new_moving_left, byte_val(SCREEN_MAX_X - 19), byte_val(DUCK_MIN_X + 8))

        new_seed_target_pos = jselect(new_seed_target_pos < 20, byte_val((SCREEN_MAX_X + 1) // 2), new_seed_target_pos)
...
@jit
def init_duck_state(s: GopherState) -> GopherState:
    return s._replace(
        duck_animation_timer=byte_val(DUCK_INIT_ANIMATION_TIMER),
        duck_render_state=byte_val(DuckRenderState.STATIONARY.value),
        seed_y=byte_val(SEED_INIT_Y),
...
@partial(jit, static_argnums=(1,))
def set_game_audio_values(s: GopherState, audio_offset: int) -> GopherState:
...
@jit
def play_game_audio(s: GopherState, carry) -> tuple[GopherState, ByteValue]:
    def play_audio(s: GopherState, audio_channel_idx: ByteValue, audio_value: ByteValue) -> tuple[
        GopherState, ByteValue]:
        a = jnp.bitwise_and(audio_value, AUDIO_DURATION_MASK)
    ...
    def play_next_audio_value(s: GopherState, audio_channel_idx: ByteValue, carry: ByteValue) -> tuple[
        GopherState, ByteValue]:
        audio_value = AUDIO_VALUES[s.audio_index_values[audio_channel_idx]]

        return jif(
            audio_value == END_AUDIO_TUNE,
...
@jit
def next_random(s: GopherState, carry: ByteValue) -> GopherState:
    r1_0 = s.random_1
    r0_0 = s.random_0
...
@jit
def update_game_state(s: GopherState) -> GopherState:
    gs = s.game_state

    s = jif_true(gs == GS_MAIN_GAME_LOOP, update_main_game_loop, operand=s)
    s = jif_true(gs == GS_GOPHER_STOLE_CARROT, carrot_stolen_by_gopher, operand=s)
    s = jif_true(gs == GS_DUCK_WAIT, wait_for_duck_to_advance_game_state, operand=s)
    s = jif_true(jor(gs == GS_RESET_AFTER_ROUND, gs == GS_INIT_NEXT_ROUND),
                 lambda s: advance_current_game_state(init_game_round_data(s), byte_val(0)), operand=s)
    s = jif_true(gs == GS_CHECK_FOR_GAME_OVER, check_for_game_over_state, operand=s)
    s = jif_true(gs == GS_FINISHED_NEXT_ROUND_SETUP, lambda s: advance_current_game_state(s, byte_val(0)), operand=s)
    s = jif_true(gs == GS_PAUSE_FOR_ACTION_BUTTON, wait_for_action_button_to_continue_round, operand=s)
    s = jif_true(gs == GS_WAIT_FOR_NEW_GAME, lambda s: end_of_frame(s, byte_val(0)), operand=s)

    return s


@jit
def update_main_game_loop(s: GopherState) -> GopherState:
    s = update_farmer_movement(s)
    s, can_be_bonked = update_gopher_movement(s)

    return jif_true(
        can_be_bonked,
        lambda s: update_gopher_animation(
...
@jit
def update_gopher_movement(s: GopherState) -> tuple[GopherState, BoolValue]:
    def move_gopher(s: GopherState) -> GopherState:
        return jif(
            s.gopher_target_move_left,
            lambda s: s._replace(
                gopher_x=jif(
                    s.gopher_x <= GOPHER_MIN_X + 1,
                    lambda _: byte_val(GOPHER_MAX_X),
            ...
            lambda s: s._replace(
                gopher_x=jif(
                    s.gopher_x >= GOPHER_MAX_X - 2,
                    lambda _: byte_val(GOPHER_MIN_X),
                    lambda s: s.gopher_x + 2,
                    operand=s
                ),
    ...
    def not_taunting(s: GopherState) -> tuple[GopherState, BoolValue]:
        ...
        return jif(
            ...
            lambda params: gopher_steal_carrot_or_move_vertically(params[0], params[1]),
...
@jit
def gopher_steal_carrot_or_move_vertically(s: GopherState, x_target: ByteValue) -> tuple[GopherState, BoolValue]:
    def steal_carrot(s: GopherState) -> GopherState:
        target_carrot = s.gopher_carrot_target_id

        return advance_current_game_state(
            s._replace(
    ...
    return jif(
        ...
        lambda params: (move_gopher_vertically(params[0], params[1]), bool_val(True)),
        operand=(s, x_target)
    )


@jit
def move_gopher_vertically(s: GopherState, x_target) -> GopherState:
    x_target = jif_true(
    ...
    y_target = GOPHER_Y_TARGET_POSITIONS[s.gopher_y_target_id]

    return jif_p2(
        s.gopher_y == y_target,
        lambda s, _: gopher_reached_vertical_target(s),
        lambda s, y_target: jif(
            s.gopher_y < y_target,
            lambda s: jif(
                s.gopher_y + 1 == GOPHER_Y_ABOVE_GROUND,
                lambda s: set_gopher_carrot_target(
                    s._replace(
...
@jit
def update_gopher_animation(s: GopherState) -> GopherState:
    y_pos = s.gopher_y

    s = jif_true(
        y_pos >= GOPHER_Y_UNDERGROUND + 7,
        lambda s: s._replace(
    ...
    s = jif_true(s.gopher_taunt_timer != 0, animate_taunting_gopher, operand=s)

    s, carry = animate_crawling_gopher(s)
    return end_of_frame(s, carry)


@jit
def animate_taunting_gopher(s: GopherState) -> GopherState:
    return s._replace(
...
@jit
def animate_crawling_gopher(s: GopherState) -> tuple[GopherState, ByteValue]:
    def animate(s: GopherState) -> tuple[GopherState, ByteValue]:
        ...
        return jif(
            ...
            lambda s: (s, jselect(s.gopher_y != GOPHER_Y_UNDERGROUND, byte_val(1), byte_val(0))),
            lambda s: (
                s._replace(
                    gopher_render_state=jselect(s.gopher_y != GOPHER_Y_UNDERGROUND,
    ...
    # No crawling
    return jif(
        jand(s.gopher_y != GOPHER_Y_UNDERGROUND, s.gopher_y != GOPHER_Y_ABOVE_GROUND),
...
@jit
def set_gopher_carrot_target(s: GopherState) -> GopherState:
...
@jit
def gopher_reached_vertical_target(s: GopherState) -> GopherState:
    return jif(
        s.gopher_y_target_id == GOPHER_Y_TARGET_UNDER_GROUND,
        set_gopher_new_target_values,
        lambda s: jif(
            s.gopher_y == GOPHER_Y_TAUNTING,
            lambda s: s._replace(
                gopher_y_target_id=byte_val(GOPHER_Y_TARGET_UNDER_GROUND),
                gopher_y_locked=bool_val(True),
                gopher_y_target_extra_flag=bool_val(False),
...
@jit
def set_gopher_new_target_values(s: GopherState) -> GopherState:
    s = next_random(s, carry=byte_val(1))
    s = s._replace(
    ...
    return normal_gopher_logic(s)


@jit
def smart_gopher_tunnel_targeting(s: GopherState) -> GopherState:
    s = jif_true(
...
@jit
def normal_gopher_logic(s: GopherState) -> GopherState:
    s = jif_true(
        s.gopher_change_direction_timer != 0,
        lambda s: s._replace(
            gopher_change_direction_timer=s.gopher_change_direction_timer - 1,
...
@jit
def update_farmer_movement(s: GopherState) -> GopherState:
    return jif_true(
        jor(s.left_pressed, s.right_pressed),
        lambda s: jif(
            s.right_pressed,
            lambda s: jif_true(
                s.farmer_x < FARMER_MAX_X,
                lambda s: s._replace(farmer_x=s.farmer_x + 1),
                operand=s
            ),
            lambda s: jif_true(
                jand(s.left_pressed, s.farmer_x >= FARMER_MIN_X),
                lambda s: s._replace(farmer_x=s.farmer_x - 1),
                operand=s
            ),
            operand=s
...
@jit
def check_for_farmer_bonking_gopher(s: GopherState) -> GopherState:
    ...
    return jif_true(
        ...
        lambda s: set_game_audio_values(increment_score(s, POINTS_BONK_GOPHER),
                                        MEM_AUDIO_BONK_GOPHER - MEM_AUDIO_VALUES)._replace(
            # Reset gopher
            gopher_x=byte_val(GOPHER_INIT_X - 4),
            # Set random new horizontal direction, and new tunnel (does not target carrot)
            gopher_target_move_left=jnp.bitwise_and(s.random_0, GOPHER_X_DIR_MASK) != 0,
            gopher_tunnel_target_id=jnp.bitwise_and(s.random_0, GOPHER_TUNNEL_TARGET_MASK),
            gopher_targeting_carrot=bool_val(False),
            gopher_carrot_target_id=byte_val(0),
...
@jit
def update_gopher_taunt_logic(s: GopherState) -> GopherState:
    s = jif_true(
        s.gopher_y == GOPHER_Y_TAUNTING,
        lambda s: jif(
            s.gopher_taunt_timer != 0,
            lambda s: s._replace(gopher_taunt_timer=s.gopher_taunt_timer - 1),
            lambda s: set_game_audio_values(s, MEM_AUDIO_GOPHER_TAUNT - MEM_AUDIO_VALUES)._replace(
...
@jit
def set_taunting_gopher_facing_direction(s: GopherState) -> GopherState:
    return jif(
...
@jit
def carrot_stolen_by_gopher(s: GopherState) -> GopherState:
    return advance_current_game_state(
        set_game_audio_values(s._replace(
            gopher_render_state=byte_val(GopherRenderState.DISABLED.value),
            frame_count=byte_val(WAIT_TIME_CARROT_STOLEN)
        ), MEM_AUDIO_STOLEN_CARROT - MEM_AUDIO_VALUES),
        carry=byte_val(0)
    )


@jit
def wait_for_duck_to_advance_game_state(s: GopherState) -> GopherState:
    return jif(
        s.duck_animation_timer != 0,
        lambda s: end_of_frame(s, carry=byte_val(0)),
...
@jit
def advance_game_state_after_frame_count_expire(s: GopherState) -> GopherState:
    return jif(
        s.frame_count != 255,
        lambda s: end_of_frame(s, carry=byte_val(0)),
        lambda s: advance_current_game_state(s, carry=byte_val(1)),
        operand=s
    )


@jit
def check_for_game_over_state(s: GopherState) -> GopherState:
    def game_over(s: GopherState):
        s = set_game_audio_values(s, MEM_AUDIO_GAME_OVER_THEME_1 - MEM_AUDIO_VALUES)
        s = set_game_audio_values(s, MEM_AUDIO_GAME_OVER_THEME_2 - MEM_AUDIO_VALUES)

        s = s._replace(game_state=byte_val(GS_WAIT_FOR_NEW_GAME))

        return end_of_frame(s, carry=byte_val(0))

    return jif(
        jnp.any(s.carrots_left),
        lambda s: advance_current_game_state(s, carry=byte_val(0)),
        game_over,
        operand=s
    )
...
@jit
def wait_for_action_button_to_continue_round(s: GopherState) -> GopherState:
    return jif(
        s.fire_pressed,
        lambda s: s._replace(
            game_state=byte_val(GS_MAIN_GAME_LOOP)
...
@jit
def advance_current_game_state(s: GopherState, carry: ByteValue) -> GopherState:
    return end_of_frame(
        s._replace(
...
@jit
def end_of_frame(s: GopherState, carry) -> GopherState:
    return s._replace(
        carry_state=carry
    )
...
def get_first_dirt_index_for_hole(s: GopherState, hole_x_position: IntValue):
    ...
    dirt_flags = jnot(
        s.garden_dirt_flags[get_garden_flag_index(get_garden_flag_x_index(hole_x_position), DIRT_LAYER_Y_INDICES)]
    )
...
class JaxGopher(JaxEnvironment[GopherState, GopherObservation, GopherInfo, None]):
    def __init__(self, reward_funcs: list[Callable[[GopherState, GopherState], chex.Array]]=None):
        super().__init__()
        ...
        self.renderer = GopherRenderer()


    @partial(jax.jit, static_argnums=(0,))
    def reset(self, key: Optional[jax.random.PRNGKey] = None, duck_enabled: bool = True,
              expert_difficulty: bool = True) -> Tuple[GopherObservation, GopherState]:
        state: GopherState = reset_to_main_loop(GopherState(), duck_enabled, expert_difficulty)
    ...
    def step(self, state: GopherState, action: int) -> tuple[
        ...
        new_state: GopherState = update_game(new_state)
    ...
    def observation_space(self) -> spaces.Space:
        return spaces.Dict({
            "farmer_x": spaces.Box(low=FARMER_MIN_X - 1, high=FARMER_MAX_X, shape=(), dtype =jnp.uint8),
            "farmer_animation_state": spaces.Box(low=0, high=2, shape=(), dtype=jnp.uint8),
            "gopher_position": spaces.Dict({
                "x": spaces.Box(low=GOPHER_MIN_X, high=GOPHER_MAX_X + 1, shape=(), dtype=jnp.int32),
                "y": spaces.Box(low=GOPHER_Y_UNDERGROUND, high=GOPHER_Y_ABOVE_GROUND, shape=(), dtype=jnp.int32)
            }),
            "duck_x": spaces.Box(low=DUCK_MIN_X - 1, high=DUCK_MAX_X, shape=(), dtype=jnp.uint8),
            ...
            "seed_position": spaces.Dict({
                # 0 when deactivated
                "x": spaces.Box(low=min(DUCK_MIN_X - 1, 0), high=DUCK_MAX_X, shape=(), dtype=jnp.int32),
                # 0 when deactivated
                "y": spaces.Box(low=min(SEED_INIT_Y, 0), high=SEED_GROUND_LEVEL, shape=(), dtype=jnp.int32)
            }),
    ...
    def _get_observation(self, state: GopherState) -> GopherObservation:
        return GopherObservation(
            ...
            hole_states=jax.vmap(lambda x: get_first_dirt_index_for_hole(state, x))(int_arr(
                [HOLE_0_X, HOLE_1_X, HOLE_2_X, HOLE_3_X, HOLE_4_X, HOLE_5_X]
            ))
        )
...
class GopherRenderer(JAXGameRenderer):
    ...
    def render(self, state: GopherState, replicate_render_artifacts: bool = True):
        ...
        raster = jnp.zeros((WINDOW_HEIGHT, WINDOW_WIDTH, 3), dtype=jnp.uint8)
        raster = self.render_sprite(raster, "background")

        # Render dug dirt
        dirt_dug_mask = jnp.broadcast_to(jnp.expand_dims(state.garden_dirt_flags[DUG_DIRT_DISPLAY_MASK], axis=-1),
                                         raster.shape)
