...
from jaxatari.games.venture.logic import (
    _update_player, _update_monsters_optimized, _update_dead_for, _update_chaser, _update_lasers,
    _check_player_hazard_collision, _handle_level_transitions
)
from jaxatari.games.venture.renderer import VentureRenderer

class JaxVentureWorld1(JaxEnvironment[GameState, VentureObservation, VentureInfo, VentureConstants]):
    ...
    def reset(self, key: jrandom.PRNGKey = None) -> tuple[VentureObservation, GameState]:
        ...
        active_monsters = indices < self.consts.LEVEL_OFFSETS[1]
        ...
        state = GameState(
            ...
            current_level=jnp.array(0, dtype=jnp.int32), world_level=jnp.array(1, dtype=jnp.int32),
    ...
    def step(self, state: GameState, action: jnp.ndarray) -> tuple[VentureObservation, GameState, jnp.ndarray, bool, VentureInfo]:
        ...
        def handle_normal_gameplay(s: GameState) -> GameState:
            ...
            p_post_trans = _handle_level_transitions(s.replace(player=s.player.replace(x=s.player.x, y=s.player.y)), self.consts)
            trans_occ = (p_post_trans.current_level != s.current_level)
            
            def normal_move():
                wall_map = self.consts.ALL_WALL_MAPS_PER_WORLD[0, s.current_level]
                t = self.consts.JAX_TRANSITIONS[0, 0]
                ...
                n_m = jax.lax.cond(jnp.mod(s.step_counter, 3) == 0, lambda: _update_monsters_optimized(s.monsters, m_key, wall_map, self.consts.MONSTER_SPEEDS[s.monster_speed_index], s.current_level, 0, s.current_level == 0, t[:, 0:4], l_portals, self.consts).replace(dead_for=_update_dead_for(s.monsters.dead_for)), lambda: s.monsters)
            ...
            def update_projectile(ps: GameState):
                ...
                wall_map = self.consts.ALL_WALL_MAPS_PER_WORLD[0, ps.current_level]
            ...
            def collect_chest(cs: GameState):
                ...
                pos = self.consts.CHEST_POSITIONS[cs.current_level]
            ...
            s = jax.lax.cond((s.current_level == 1), lambda _s: _s.replace(lasers=_update_lasers(_s.lasers, self.consts.LASER_SPEED, self.consts.LASER_BOUNDS)), lambda _s: _s, s)

            # Hazards
            if_coll = _check_player_hazard_collision(s.player, s.monsters, s.chaser, s.lasers, s.current_level, 1, self.consts)
    ...
    def _get_observation(self, state: GameState) -> VentureObservation:
        ...
        t = self.consts.JAX_TRANSITIONS[0, state.current_level]
        ...
        c_pos = self.consts.CHEST_POSITIONS[state.current_level]
        ...
        is_laser = (state.current_level == 1)
        l_x = jnp.array([state.lasers.positions[0], state.lasers.positions[1], 80.0, 80.0])
        l_y = jnp.array([105.0, 105.0, state.lasers.positions[2], state.lasers.positions[3]])
        lasers = ObjectObservation.create(x=clip_x(jnp.where(is_laser, l_x, -1.0)), y=clip_y(jnp.where(is_laser, l_y, -1.0)), width=clip_x(jnp.array([4.0, 4.0, 20.0, 20.0])), height=clip_y(jnp.array([20.0, 20.0, 4.0, 4.0])), active=jnp.where(is_laser, jnp.ones(4, dtype=jnp.int8), jnp.zeros(4, dtype=jnp.int8)), orientation=jnp.zeros(4))
